...
% select tranmitting / receiving elements 
...
theta = 0;                      % azimuth in deg
r = (1:exct.n_sampl)';
index = 1; 
Ctot = zeros(length(theta),exct.n_sampl); 
for i = [5]
    array.v = ones(size(array.X));
    array.u = zeros(size(array.X));
    array.u(i) = 1; 
    ...
    plot(real(im.C))
    %subplot(1,2,index)
%     pcolor(im.x, im.y, abs(Ctot)'), shading flat, axis equal tight, axis([-1500, 1500, -1500,1500])
%     title('DAS result')
%     colorbar 
