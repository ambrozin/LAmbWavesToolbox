function im = plane_wave_DAS_ULA_phs_disp_compens(array, Res, theta, r, wave, exct,disp_data) 
...
for i = 1:length (theta) 
   ...
   % delay in samples 
   delaySampl = round( array.X .* cosd(theta(i))*1e-3 /dx );
   Res2 = zeros(size(Res));
...
%    figure(3), plot(real(Res3)'), xlim([500 1200]), drawnow
   ...
   im.C(i,:) = abs(sum(Res2(:,1:size(im.x,1))));
   im.coh_C(i,:) = C_coh(:,1:size(im.x,1));
   
   im.Cphs(i,:) = abs(sum(Res3(:,1:size(im.x,1))));
   im.coh_Cphs(i,:) = Cphs_coh(:,1:size(im.x,1));
