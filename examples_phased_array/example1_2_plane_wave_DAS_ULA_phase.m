...
arrayRes = simulate_array_response(array.X,array.Y,target, exct,f_vec,k_a, k_a);
...
data = -im.coh_Cphs + min(min(im.coh_Cphs));
...
plot(theta, (im.Cphs(:,indCphs)./mCphs),theta,(BP_coh_Cphs_image./max(BP_coh_Cphs_image)))
xlabel('angle [\circ]'), xlim([0 180])
ylabel('BP [a.u.]'), %ylim([-40 0])
hlgd = legend('Discrete-time DAS','Phase coherence weighted'); 
set(hlgd, 'location', 'northoutside', 'orientation', 'horizontal', 'box', 'off')